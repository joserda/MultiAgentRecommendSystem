...
class ProductRecAgent(BaseAgent):
    def __init__(self, vector_store: MilvusVectorStore | None = None):
        ...
        self._use_vector_store = False  # 是否启用向量检索
    ...
    async def _recall(self, profile: UserProfile | None, limit: int) -> list[Product]:
        """
        多策略召回：向量检索 + 协同过滤 + 热门商品.
        
        优先级：
            1. 向量检索 (Milvus)：基于用户画像偏好向量召回
            2. 协同过滤：基于用户历史行为召回
            3. 热门/新品兜底
        """
        ...
        # 策略1：向量检索 (如果可用)
        if self._use_vector_store and self.vector_store and profile:
            try:
                # 基于用户偏好类目和最近浏览进行向量召回
                hits = await self.vector_store.search_by_user_profile(
                ...
                logger.info("recall.vector_success", count=len(candidates))
            except Exception as e:
                logger.warning("recall.vector_failed", error=str(e))

        # 策略2：热门商品兜底 (向量检索结果不足或不可用)
        if len(candidates) < limit:
            mock_products = list(MOCK_PRODUCTS)
            if profile and profile.preferred_categories:
                preferred = set(profile.preferred_categories)
                mock_products.sort(
                    key=lambda p: (
                        p.category in preferred,  # 偏好类目优先
                        "新品" in p.tags,  # 新品加权
                        p.stock > 0,  # 有库存优先
                        random.random(),
                    ),
            ...
            candidates.extend(mock_products[: limit - len(candidates)])
